...
class BTree<E> {
    ...
    public boolean isConsecutive() {
        return isConsecutivePreOrder(root, root.info);
    }

    //TODO
    private boolean isConsecutivePreOrder(BNode<E> node, E prev) {
        if (node == null) {
            return true;
        }
        if (node.left != null) {
            if (node.left.info.equals(prev)) {
                return isConsecutivePreOrder(node.left, prev);
            } else if (node.left.info.equals((Integer) prev + 2)) {
                return isConsecutivePreOrder(node.left, node.left.info);
            } else {
                return false;
            }
        }
        if (node.right != null) {
            if (node.right.info.equals(prev)) {
                return isConsecutivePreOrder(node.right, prev);
            } else if (node.right.info.equals((Integer) prev + 2)) {
                return isConsecutivePreOrder(node.right, node.right.info);
            } else {
                return false;
            }
        }
...
public class ConsecutiveNumbers {
    ...
    public static void main(String[] args) throws Exception {
        ...
        boolean isConsecutive = tree.isConsecutive();
        System.out.println(isConsecutive ? "true" : "false");
    }

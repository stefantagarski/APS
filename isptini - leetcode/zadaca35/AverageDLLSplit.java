...
public class AverageDLLSplit {

    private static void splitAvg(DLL<Integer> list) {
        ...
        DLL<Integer> smaller = new DLL<>();
        DLL<Integer> bigger = new DLL<>();

        last = list.getLast();
        while (last != null) {
            if (last.element <= avg) {
                smaller.insertLast(last.element);
            }else {
                bigger.insertLast(last.element);
            }
            last = last.pred;
        }

...
public class Race {
    ...
    private static void competition(SLL<Athlete> prevWinners, SLL<Athlete> currYearRunners) {
       SLLNode<Athlete> prev = prevWinners.getFirst();
       SLLNode<Athlete> curr = currYearRunners.getFirst();

       double max = 0;
       while (prev != null) {
           if (prev.element.getTime() > max) {
               max = prev.element.getTime();
           }
           prev = prev.succ;
       }

       while (curr != null) {
           if (curr.element.getTime() > max) {

...
public class Tasks{
    public static void work(SLL<Task> toDo, SLL<Task> inProgress) {
       ...
       while (nodeTODO != null) {
           int importance = 2 * nodeTODO.element.getPriority() * nodeTODO.element.getPriority();
           if (importance > max) {
               max = importance;
               mostImportant = nodeTODO;
           }
       ...
       SLLNode<Task> nodeINP = inProgress.getFirst();
       SLLNode<Task> leastImportant = null;
       int min = 9999;

       while (nodeINP != null) {
           int importance = 2 * nodeINP.element.getPriority() * nodeINP.element.getPriority();
           if (importance < min) {
               min = importance;
               leastImportant = nodeINP;
           }
           nodeINP = nodeINP.succ;
       }

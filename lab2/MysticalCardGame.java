...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> maxSignificanceCardNode = firstSorcererCards.getFirst();
        SLLNode<Card> currentCardNode = firstSorcererCards.getFirst().succ;

        while (currentCardNode != null) {
            if (currentCardNode.element.significance() > maxSignificanceCardNode.element.significance()) {
                maxSignificanceCardNode = currentCardNode;
            }
            currentCardNode = currentCardNode.succ;
        }

        firstSorcererCards.delete(maxSignificanceCardNode);

        SLLNode<Card> middleNode = secondSorcererCards.getFirst();
        for (int i = 0; i < secondSorcererCards.size() / 2; i++) {
            middleNode = middleNode.succ;
        }
        secondSorcererCards.insertBefore(maxSignificanceCardNode.element, middleNode);
    }
